...
function processBlueskyStatus(resultContent: string, entryUrl: string, entryImageUrl: string, entryTitle: string, entryAuthor: string): { trimmedContent: string; needsEllipsis: boolean; urlToShow: string } {
  ...
  let needsEllipsis = bsTrimmed.needsEllipsis;
  // 2. Determine URL visibility: forced, truncated, or Bluesky quote post
  const isQuote = isQuoteInPost(resultContent, '', 'BS');
  const shouldShowUrl = SETTINGS.SHOW_ORIGIN_POSTURL_PERM || needsEllipsis || isQuote;
  let urlToShow = '';
...
function processDefaultStatus(resultContent: string, entryUrl: string, entryImageUrl: string, entryTitle: string, entryAuthor: string): { trimmedContent: string; needsEllipsis: boolean; urlToShow: string } {
  // 1. Trim the content according to SETTINGS.POST_LENGTH and strategy
  const defaultTrimmed = trimContent(resultContent);
  let trimmedContent = defaultTrimmed.content;
  let needsEllipsis = defaultTrimmed.needsEllipsis;
  // 2. Decide if the original URL should be shown (forced or truncated)
  const shouldShowUrl = SETTINGS.SHOW_ORIGIN_POSTURL_PERM || needsEllipsis;
  let urlToShow = '';
...
function processTwitterStatus(resultContent: string, entryUrl: string, entryImageUrl: string, entryTitle: string, entryAuthor: string): { trimmedContent: string; needsEllipsis: boolean; urlToShow: string } {
  ...
  let trimmedContent = trimmedResult.content;
  let needsEllipsis = trimmedResult.needsEllipsis;
  // 3. Detect any repost URL
  const repostUrl = findRepostUrl(resultContent);
  const isRepostUrlDetected = repostUrl !== null;
  // 4. Possibly force showing origin URL if no URLs remain and not a media page
  const isMediaPage = typeof entryImageUrl === 'string' && (entryImageUrl.endsWith('/photo/1') || entryImageUrl.endsWith('/video/1'));
  if (!isUrlIncluded(cleanedContent) && !isMediaPage) { SETTINGS.SHOW_ORIGIN_POSTURL_PERM = true; }
  // 5. Determine if URL should be shown based on multiple conditions
  const isExternalRepost = isRepost(entryTitle) && !isRepostOwn(entryTitle, entryAuthor) && SETTINGS.REPOST_ALLOWED;
  const shouldShowUrl = isRepostUrlDetected || SETTINGS.SHOW_ORIGIN_POSTURL_PERM || needsEllipsis || isExternalRepost || isMediaPage;
  // 6. Select which URL to display (prefer image URL if present)
  let urlToShow = '';
